...
template <typename COMTYPE>
std::vector<COMTYPE> Communicator<COMTYPE>::cvec = std::vector<COMTYPE>(1,
                                                                        COMTYPE::getWorldContext());
class MPIContext {
    MPIContext(){};
  ...
  public:
    ...
    {
        if (slice > ctx.size) {
            throw std::invalid_argument("Cannot Split MPI Context:Not Enough Space in MPI Context");
        }
        if (slice > 0) {
