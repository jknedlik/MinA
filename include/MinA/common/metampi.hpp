...
struct MPITypes<index, T> {
    static void fill(decltype(MPI_DOUBLE) * types) { types[index] = MPITYPE<T>::value; }
};

template <size_t index, typename Tuple>
void fillarr(Tuple& tup, MPI_Aint* arr, MPI_Datatype* darr, std::false_type::type)
{
...
template <size_t index, typename Tuple>
void fillarr(Tuple& tup, MPI_Aint* arr, MPI_Datatype* darr, std::true_type::type)
{
    auto a = (long)(&std::get<index>(tup));
    auto b = (long)(&std::get<0>(tup));
    darr[index] = MPITYPE<std::remove_reference_t<decltype(std::get<index>(tup))>>::value;
    arr[index] = b - a;
    fillarr<index + 1, Tuple>(
      tup, arr, darr,
      typename std::integral_constant<bool, (index + 1 < std::tuple_size<Tuple>::value)>::type());
}
...
template <typename Tuple>
Tuple mpi_message(Tuple& tup, int source, int destination, auto comm = MPI_COMM_WORLD, int tag = 0)
{
    int rank;
