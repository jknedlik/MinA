...
class Simplex : public MinA::Algorithm<ai<Function>, SimplexMeta, Function> {
  public:
    static constexpr size_t mDimension = std::tuple_size<typename Function::parametertype>::value;
    Simplex() : MinA::Algorithm<ai<Function>, SimplexMeta, Function>::Algorithm() { reset(); }
    virtual void reset()
    {
        this->restored = false;
        this->mMetaBoundaries =
          decltype(this->mMetaBoundaries){ { 0.0, 2.0 }, { 0.0, 1.0 }, { 0.0, 2.0 }, { 0.0, 1.0 } };
    ...
    void setAlpha(double a) { std::get<SIMPLEXT_ALPHA>(this->mMetaParameters) = a; }
    ...
    void setGamma(double g) { std::get<SIMPLEXT_GAMMA>(this->mMetaParameters) = g; }
    vertex<Function> getCentroid(simplext<Function> A)
    {
        // Return centroid M of all vertices, but the nExcluded worst ones, that is the centroid
        // of
        ...
        // the worst one.
        // The vertices of A are supposed to be already ordered by function value, from best to
        // worst
        //
        vertex<Function> M;
        ...
        // (this->mpi_procs));
        // });
        for_each_tuple(M.first, [this](auto& l) { l /= (mDimension + 1 - (this->mpi_procs)); });
    ...
    vertex<Function> getShrinkedPoint(vertex<Function>& Ap, vertex<Function>& A0)
    {
        ...
        for_each_tuple(Ap.first, A0.first, Anew.first, [&](auto ap, auto a0, auto& anew) {
    ...
    double getSimplexSize(simplext<Function>& A)
    {
        ...
        vertex<Function> M = getCentroid(A); // the centroid of the simplex, no vertex excluded
        for (int i = 0; i < A.size(); i++) {
            double vertexCentroidDistance = 0.;
            for_each_tuple(A[i].first, M.first,
                           [&](auto a, auto m) { vertexCentroidDistance += pow(m - a, 2); });
    ...
    void setStepSize()
    {
        for_each_tuple(std::get<SIMPLEX_AI_STEPSIZES>(this->mAlgorithmInformations),
                       this->f.startvalues, this->f.bounds, [](auto& step, auto start, auto bound) {
    ...
    void printOutVertex(vertex<Function>& simplexVertex, string vertexName, ostream& outStream)
    {
        ...
        for_each_tuple(simplexVertex.first,
                       [&outStream](auto x) mutable { outStream << setw(columnWidth) << x; });

        outStream << "   f(" << vertexName << ")= " << setw(columnWidth) << simplexVertex.second
                  << "\n";
    }
