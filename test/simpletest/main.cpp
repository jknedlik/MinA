...
int main()
{
    ...
    for_eachn(std::tie(xr, yr),
              [](auto& xx, auto& yy) { std::cout << xx << " " << yy << std::endl; });

    //  for_each_tuple(xr, yr, [](auto& tuple, auto& t2) { std::cout << tuple << t2; });
    // for_each_tuplen(xr, yr, [](auto& xx, auto& yy) { std::cout << xx << " " << yy << std::endl;
    // });
    ...
    */
    ...
    for_each_tuple(testt, testtt,
                   [](auto& t, auto& s) { std::cout << t << "ttt" << s << std::endl; });
    Boundarytuple<decltype(testt)>::type testboundary;
    for_each_tuple(testboundary, [&](auto& b) {
        b.left = 0;
        b.right = 1;
    });
    for_each_tuple(testboundary, [&](auto b) { std::cout << b.left << "<-l r->" << b.right; });

    for_each_tuple(megat, [](auto x) { for_each_tuple(x, [](auto y) { printmax(y); }); });
    TupleVector<std::tuple<int, int, double>>::type x{ { 11, 12, 13, 14 }, { 21 }, { 31.5, 32.5 } };
    ...
    int rank;
    ;
    std::cout << MPI_INT << "i" << MPI_DOUBLE << "d" << MPI_CHAR << "c" << std::endl;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    std::tuple<double, int, double, int, int, int, int> mpitup;
    std::cout << "Mpitupsize: " << sizeof(mpitup) << std::endl;
    std::cout << "Char,Double,int" << MPI_CHAR << " " << MPI_DOUBLE << " " << MPI_INT << std::endl;
