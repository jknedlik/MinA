...
{
    {
        ...
        if (cx) {
            ...
            mc.f.alg.setFileName(".Multi.single" + std::to_string(cx.getIdent()) + ".psimplex" +
                                 mc.f.alg.f.fn);
...
{
    {
        ...
        if (cx) {
            ...
            mc.f.alg.setFileName(".Multi.single" + std::to_string(cx.getIdent()) + ".simplex" +
                                 mc.f.alg.f.fn);
...
{
    size_t s = 1;
    for (int i = 0; i < std::tuple_size<typename Func::parametertype>::value; i++) {
        s *= (i < 5) ? 2 : 1;
    }
