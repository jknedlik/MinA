...
class SquareFunction:public FunctionToBeOptimized{
	...
	double sum_square=0;	
	for (auto it : parameters) sum_square+=para[it.getName()]*para[it.getName()];	
	return sum_square;
	}
...
class McCormick_function:public FunctionToBeOptimized{
	...
	double getEvaluation(std::map <string, double> para){
	double fz;
	fz=sin(para["x"]+para["y"])+pow(para["x"]-para["y"],2)-1.5*para["x"]+2.5*para["y"]+1;
	return fz;
	};
};
...
class Schwefel_function:public FunctionToBeOptimized{
	...
	double getEvaluation(std::map <string, double> para){
		double sum_xsin=0;
		for (auto it : parameters) sum_xsin+=para[it.getName()]*sqrt(abs(para[it.getName()]));
